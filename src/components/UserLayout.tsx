  ...
  return (
    <div className="flex min-h-screen bg-[#059669] pb-16 lg:pb-0">
        ...
        <div className="flex h-[calc(100vh-8rem)] flex-col justify-between p-4">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  to={item.path}
                  className={`relative flex items-center space-x-3 rounded-full px-5 py-3.5 text-sm font-semibold transition-all ${
                    isActive 
                      ? 'text-[#059669]' 
          ...
          <div className="border-t border-[#10b981] pt-4">
            <div className="mb-4 px-4">
              ...
              <p className="text-xs text-emerald-100 truncate">{userProfile?.email}</p>
            </div>
            <button
              onClick={handleLogout}
              className="flex w-full items-center space-x-3 rounded-full px-5 py-3 text-sm font-semibold text-emerald-50 hover:bg-rose-50 dark:bg-rose-950/500 hover:text-white transition-colors"
            >
      ...
      <main className="flex flex-1 flex-col overflow-y-auto lg:pl-64 overflow-x-hidden">
        ...
        {/* Desktop Header */}
        <header className="hidden lg:flex sticky top-0 z-10 h-16 items-center justify-end bg-transparent px-8 gap-3 mt-2">
